...
## Tabela-resumo (7 Set 2026)

| Motor | Ads? | Canal de compra | Geografia |
|---|---|---|---|
| ChatGPT | Sim | OpenAI Ads Manager (self-serve — **estado disputado para PT, ver nota**); primeira marca PT confirmada (Continente, 4 Set) comprou via agência, não self-serve | Ads live desde 24 Ago 2026 em 31 mercados EEE+Suíça, incl. Portugal (Free e Go); self-serve Ads Manager **reportado** em beta desde 1 Set, com a página de elegibilidade da OpenAI a aparentar ter mudado Portugal para "Available" a 2 Set (soku.ai, Unite.AI, 52 países) — mas fontes portuguesas dedicadas não actualizadas desde então continuam a descrever acesso só via agência/Cosmo5; sem confirmação de primeira mão de nenhum dos dois lados, prazo de verificação hoje (7 Set), ver secção ChatGPT |
...
- **Estado UE anunciantes — DISPUTADO, mas com novo peso de evidência (actualizado 7 Set 2026, no próprio dia do prazo):** a leitura de 1 Set (hiato fechado, self-serve aberto para os 31 mercados incl. Portugal) já não é o único relato, mas hoje encontrou-se um dado que a reforça: a própria página de elegibilidade da OpenAI (help.openai.com/en/articles/20001245) aparenta ter mudado o estado de Portugal para "Available" a **2 Set 2026**, segundo dois rastreadores independentes (soku.ai — "ChatGPT Ads Self-Service Just Opened in 52 Countries" — e Unite.AI), que descrevem a contagem de países self-serve a subir de 9 para 52 nesse dia, com Portugal nomeado explicitamente pelo soku.ai. Nenhum dos dois acedeu à página em causa por conta própria de forma verificável nesta sessão — `WebFetch` a help.openai.com continua bloqueado, achado via `WebSearch`. Em paralelo, fontes portuguesas dedicadas (leak.pt, adclickgroup.com, briefing.pt sobre a Cosmo5, ECO sobre a Continente) **não foram actualizadas desde 2 Set** e continuam a descrever acesso só via agência/Cosmo5/equipa Ads Solutions, com self-serve "a chegar nos próximos meses" — isto pode ser leitura correcta e persistente, ou pode ser imprensa PT desactualizada face a uma mudança de plataforma recente. **Nenhuma fonte, de nenhum dos dois lados, reporta um caso em primeira mão (fórum, rede social, notícia) de uma conta portuguesa a chegar ou a ser bloqueada no ecrã de criação de campanha em ads.openai.com** — esse continua a ser o único facto que fecha a questão, e continua por obter.
- **Verificação de primeira mão (URGENTE — hoje é o prazo, 8 Set 2026 é amanhã):** sem acesso a uma conta ChatGPT Free/Go nem a uma conta Ads Manager geolocalizada em Portugal nesta sessão (ambiente sandbox, sem browser autenticado) — nem o "primeiro anúncio visível" nem o "primeiro registo bem-sucedido de anunciante PT" estão confirmados em primeira mão. A rotina não tem, e nunca teve nas últimas sessões, via própria para fazer este teste. Com o prazo a fechar amanhã e um sinal plausível (mas não confirmado) de que o hiato pode já ter fechado a 2 Set, a acção que resta é humana: alguém com acesso a browser normal tenta o registo real em ads.openai.com com país Portugal e regista onde/se bloqueia, per o procedimento de `daily-prompt.md` § "Verificação de primeira mão" — ver alerta ao founder no topo do news-feed.
