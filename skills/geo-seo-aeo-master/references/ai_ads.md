...
|---|---|---|---|
| ChatGPT | Sim | OpenAI Ads Manager (self-serve — **estado disputado para PT, ver nota**); duas marcas PT confirmadas (Continente, 4 Set; Robbialac, ~1-6 Set) compraram via agência, nenhuma por self-serve; até o parceiro oficial da OpenAI em PT (Cosmo5) está só "a preparar" a sua primeira activação | Ads live desde 24 Ago 2026 em 31 mercados EEE+Suíça, incl. Portugal (Free e Go); self-serve Ads Manager **reportado** em beta desde 1 Set, com a página de elegibilidade da OpenAI a aparentar ter mudado Portugal para "Available" a 2 Set (soku.ai, Unite.AI, 52 países) — mas fontes portuguesas dedicadas não actualizadas desde então continuam a descrever acesso só via agência/Cosmo5; sem confirmação de primeira mão de nenhum dos dois lados, prazo original de verificação era 8 Set (hoje), passou sem resolução, ver secção ChatGPT |
...
- **Estado UE anunciantes — DISPUTADO, prazo original chega hoje sem resolução (actualizado 8 Set 2026):** a leitura de 1 Set (hiato fechado, self-serve aberto para os 31 mercados incl. Portugal) continua a coexistir, sem se resolver, com a leitura das fontes portuguesas dedicadas ("ainda fechado, só por agência/Cosmo5"). Dois dados novos hoje reforçam, sem confirmar, o lado "ainda fechado": (1) o briefing.pt sobre a Cosmo5 — o parceiro oficial designado pela OpenAI para a fase de acesso alfa em Portugal e Espanha — diz que a própria Cosmo5 está ainda só "a preparar" uma primeira activação doméstica; se o parceiro que a OpenAI escolheu para testar o mercado português ainda não completou uma campanha, isso pesa contra a leitura de que o self-serve já está a funcionar livremente aqui, mesmo que a página de elegibilidade diga "Available"; (2) uma segunda marca PT nomeada — Robbialac, via a agência Social Animals — lançou campanha no ChatGPT, de novo por via de agência, não por registo directo. **Continua sem existir, de nenhum dos dois lados, um único caso em primeira mão de uma conta PT a chegar ao ecrã de criação de campanha em ads.openai.com, ou a ser bloqueada nele** — nem via Cosmo5, nem via nenhum anunciante independente, nem confirmação directa da própria página help.openai.com/en/articles/20001245 (`WebFetch` continua bloqueado).
- **Verificação de primeira mão — o prazo original (8 Set 2026) chega hoje sem se fechar:** sem acesso a uma conta ChatGPT Free/Go nem a uma conta Ads Manager geolocalizada em Portugal nesta sessão (ambiente sandbox, sem browser autenticado), a rotina confirma pela terceira sessão consecutiva que não tem via própria para este teste. A acção que resta é humana: alguém com acesso a browser normal tenta o registo real em ads.openai.com com país Portugal e regista onde/se bloqueia, per o procedimento de `daily-prompt.md` § "Verificação de primeira mão". Sem essa tentativa, mais dias de research indirecta não vão mudar o quadro — ver alerta ao founder no topo do news-feed, que recomenda ao founder decidir entre tentar o registo ele próprio ou aceitar a leitura mais plausível (self-serve aberto desde 2 Set) sem confirmação de primeira mão.
