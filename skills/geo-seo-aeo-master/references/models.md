...
|---|---|---|---|
| **DeepSeek V4.1-Flash** | 10 Sep 2026 | not confirmed | `deepseek-flash`, 552B MoE backbone, causal encoder-decoder architecture, up to 1M-token context. All `deepseek-v4-pro` traffic routes to it from 04:00 UTC 14 Sep 2026. Not verified by direct fetch (`api-docs.deepseek.com/news/news260910/` identified but `WebFetch` blocked this session); corroborated by 3+ independent secondary sources (Dataconomy, TechBriefly, eesel AI, DeepInfra, CellCog) converging on the same specs and pricing. Not yet reflected in the Deck Builder mapping below: see caveat there, to revisit after the 14 Sep migration |
| **DeepSeek V4-Flash-Vision-Exp** | API live 21 Aug 2026; open weights (MIT) 31 Aug 2026 | Yes | Multimodal variant, 284B total/13B active MoE, 1M-token context; not yet reflected in the Deck Builder mapping below (still `deepseek-v4-flash`) |
...
**Caveat: `deepseek` row (12 Set 2026, watch, não aplicado ainda):** DeepSeek-V4.1-Flash (`deepseek-flash`, 10 Set 2026) tem migração de tráfego de `deepseek-v4-pro` anunciada para 14 Set 2026; não confirmado por leitura direta (`WebFetch` bloqueado, ver secção DeepSeek acima). As colunas `production`/`cost_optimized` abaixo mantêm-se em `deepseek-v4-flash` até essa data passar e a mudança poder ser confirmada; revisitar depois de 14 Set.
