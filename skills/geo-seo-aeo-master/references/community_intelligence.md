...
negative for atual criticism, not for mere absence of enthusiasm.
...
Otimizar a marca para a IA É otimizar o todo; a queixa operacional de
hoje é a resposta negativa do modelo no próximo ciclo.

**Regras de extração (em `analyze_community`):**
...
antes de qualquer acesso à API, por formulário manual; projetos
...
2. **Threads que os motores citam** nas respostas medidas: a via mais
   valiosa: dá o subreddit exato que decide a categoria do cliente.
