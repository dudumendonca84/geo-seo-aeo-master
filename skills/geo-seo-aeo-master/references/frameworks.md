...
### Watch, corroborated but still not primary-source-verified: Cloudflare default-blocking AI crawlers on ad-supported pages (from 15 Sep 2026)

Upgraded 12 Set 2026: still not verified by direct fetch to Cloudflare's own blog (`EGRESS_BLOCKED` again today), but now corroborated by 5+ independent secondary outlets (chudi.dev, crawl-lab.com, fastcrw.com, aichatdaily.com, proxycove.com) describing the identical mechanism with consistent detail: Training and Agent crawlers get blocked by default on ad-monetized pages for new/free-tier Cloudflare customers from **15 Sep 2026** (Search crawlers unaffected). One outlet (technology.org) dates the original announcement to 3 Jul 2026, which reframes this less as breaking news and more as a previously-announced change about to take effect. Unlike Content Signals above, this would be an actual technical block, not a declared-intent flag, and would sit at the CDN layer in front of any site on Cloudflare regardless of that site's own `robots.txt`. **Still not treated as confirmed fact for methodology purposes** pending a direct read of blog.cloudflare.com, but the corroboration is now strong enough to flag as actionable: check directly on or after 15 Set 2026, and if confirmed this is a §2-level methodology trigger (a new default crawler-access rule sitting in front of GPTBot/ClaudeBot/OAI-SearchBot/PerplexityBot for any client on Cloudflare) needing a `methodology-changelog.md` entry, not just a reference update.
