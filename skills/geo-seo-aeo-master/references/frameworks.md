...
### Watch, not yet confirmed: Cloudflare default-blocking AI crawlers on ad-supported pages (from 15 Sep 2026)

Single-sourced (`WebSearch` summary only, 11 Sep 2026 daily-agent research pass; not independently corroborated, direct fetch to Cloudflare's own blog blocked this session): reported that Cloudflare will start default-blocking AI training/agent crawlers on ad-supported pages from **15 Sep 2026**, unless the AI company separates its training crawler from its non-training crawler. Unlike Content Signals above, this would be an actual technical block, not a declared-intent flag, and would sit at the CDN layer in front of any site on Cloudflare regardless of that site's own `robots.txt`. **Not treated as fact until corroborated by a second source or Cloudflare's own blog**: flagged here so the next daily pass checks blog.cloudflare.com directly around 15 Sep 2026. If confirmed, this is a §2-level methodology trigger (a new default crawler-access rule sitting in front of GPTBot/ClaudeBot/OAI-SearchBot/PerplexityBot for any client on Cloudflare) and would need a `methodology-changelog.md` entry, not just a reference update.
...
Maintained for: destaque.ai. Last refresh: 11 Sep 2026 (§2: flagged, unconfirmed watch on Cloudflare default-blocking AI crawlers on ad-supported pages from 15 Sep 2026; §5: deeper read of the 4 Sep Peec.ai "Labrador" post, same source, more mechanism detail, still WebSearch-summary only). Previous: 11 Aug 2026.
