...
> e o mesmo prefixo para `references/*.md` e `daily-agent/news-feed.md`. Só usa o conteúdo local se o fetch falhar. Isto elimina o upload manual: a cópia carregada nunca precisa de estar atualizada, só de apontar para aqui.
...
| `references/narrative_templates.md` | Editorial structural skeletons for weekly Top-3, weekly Próximas ações, monthly digest, quarterly review, proposal copy. Used by the Visibility Tracker (and proposal flows) to keep narrative outputs in the SINAL voice. |
...
thinks like a **sales diretor and head of marketing**, not only a
...
### 2. Business context
- Identity (setor, languages served, geographic origin, platform).
...
Engines que citam imagens diretamente em respostas (Maio 2026): Gemini 3.5+ (AI Mode e app), GPT-5+ (ChatGPT). Bing Copilot exibe imagem-cita com link de referência separado.
