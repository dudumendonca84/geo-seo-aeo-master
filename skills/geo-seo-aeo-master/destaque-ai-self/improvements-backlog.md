...
_(Atualizado 21 setembro 2026: nona execução do Routine, segunda semana consecutiva sem hiato. Duas correções confirmadas desde 14 set, movidas para DONE: o `llms.txt` já não descreve `/en/about` como "consultancy" (fix aplicado no mesmo dia da última auditoria), e o motor ChatGPT do Tracker não regista nenhuma nova ocorrência de erro numa janela de 7 dias (crédito Gemini também recuperado, mesma leitura de causa provável, ambos movidos para DONE com a reserva de que é leitura por ausência de sintoma, não confirmação direta de billing). Do lado que piora: Perplexity sobe de 50 para 165 ocorrências de rate-limit (429) na mesma janela; DataForSEO/`google_aio` continuam degradados e sobem de prioridade. ENTITY: `Organization.sameAs` continua sem o Wikidata, 7ª semana sem progresso, sem nova pesquisa dedicada esta semana. STRATEGIC: quarto eixo de colisão do acrónimo "GEO" confirmado, desta vez com outsourcing/nearshoring de TI em resultados de pesquisa. Novo item de baixo risco: bug isolado em `/settings`, sem recorrência desde 15 set.)_
...
- **Notes:** O mais grave de toda a auditoria: não é um achado sobre o site público, é uma falha operacional no produto vivo, a decorrer há pelo menos 3 dias antes desta auditoria. **Atualização 2026-09-21: DONE.** `mcp__Vercel__get_runtime_errors` (7 dias) não mostra nenhuma ocorrência de `gpt-5.5-instant` nem de `gpt-5.6-sol` sem crédito. Candidato a causa: deploy `#409` no `destaque-ai-tracker` ("Conta sem saldo é porta fechada, e o cartão volta a dizer o que falhou", produção 2026-09-16 06:52 UTC), por proximidade temporal com a última ocorrência (14 set 19:10 UTC) e pela descrição do commit. **Reserva:** é leitura por ausência de sintoma mais correlação de datas, não confirmação directa de billing; reabrir se reaparecer.

### 2026-09-14 added: MEASUREMENT: Tracker: Perplexity com duas assinaturas de falha distintas
- **Prioridade:** P1 (piora: volume mais do que triplicou, ver actualização)
- **Esforço:** 1-2h (diagnóstico: pode ser quota a rever e/ou throttling a implementar)
- **Origem:** auditoria semanal 2026-09-14 (`mcp__Vercel__get_runtime_errors`, projeto `destaque-ai-tracker`, janela 7 dias); revisto 2026-09-21
- **Estado:** TODO
...
- **Verificação:** `mcp__Vercel__get_runtime_errors` não mostra novas ocorrências de nenhuma das duas assinaturas numa janela de 7 dias.
- **Notes:** Duas assinaturas distintas no mesmo motor, uma delas (401) já com 6 semanas em aberto sem correção. **Atualização 2026-09-21: piora, não recuperação.** O rate-limit 429 sobe para **165 ocorrências, 6 utilizadores**, activo até 2026-09-21T07:06:29Z (menos de duas horas antes desta auditoria): quase quatro vezes o volume de 14 set. A assinatura de quota (401) não aparece na janela desta semana: pode estar resolvida ou apenas fora da janela de 7 dias, não decidível com este dado. Ao contrário do ChatGPT e do Gemini (ambos recuperados na mesma janela, ver itens DONE), o Perplexity não beneficiou da correcção aplicada a 16 set: candidato a problema distinto (limite de plano/quota do Perplexity em vez de crédito esgotado).
...
- **Origem:** auditoria semanal 2026-09-14 (extração programática do JSON-LD da homepage, comparada com a confirmação direta de 2026-08-10); reconfirmado 2026-09-21
- **Estado:** TODO
...
- **Notes:** Achado de alta confiança (comparação direta de duas extrações programáticas, 10 ago vs. 14 set); a causa exata não é decidível só pelo schema. **Atualização 2026-09-21:** reconfirmado por extracção directa (ainda 3 entradas, sem Wikidata); sem pesquisa dedicada nova ao Wikidata esta semana (gestão de tempo), pelo que o "porquê" continua tão por decidir como há uma semana. 7ª semana desde a confirmação directa de 4 entradas (10 ago), sem decisão do founder registada.
...
- **Notes:** Primeira confirmação desta colisão específica por este Routine: só uma pesquisa até agora, tratar como achado a confirmar, não como padrão estabelecido (regra de duas auditorias antes de agir com convicção, per precedente do achado "GEO vs. geodesia"). **Atualização 2026-09-14: `LR2` reconfirmado pela 2ª vez** (AEO Consulting Ltd, EmasConsultors, Morley Consulting, GRC Customs: todas conformidade aduaneira). **Terceira colisão distinta do acrónimo "GEO" descoberta**: o prompt `V3` ("vale a pena fazer outsourcing de GEO numa empresa de software em Portugal?") resolveu inteiramente para "Global Employer of Record" (outsourcing de RH/payroll: ensun.io, Ascendix, Newxel, Accelerance): zero relevância a Generative Engine Optimization. Três eixos de colisão confirmados agora: geodesia/topografia (PT, 4ª semana), "Authorized Economic Operator" (EN, 2ª semana), "Global Employer of Record" (EN, 1ª semana). **Atualização 2026-09-21: quarto eixo confirmado, e desta vez no mesmo prompt `V3`.** Repetido esta semana via `WebSearch`, `V3` volta a não devolver destaque.ai: desta vez a leitura é "outsourcing/nearshoring de TI genérico" (guias sobre contratar programadores em Portugal), não "Global Employer of Record" como na semana anterior: o mesmo prompt colide com sentidos diferentes de "GEO" consoante a formulação exacta da pesquisa, o que é em si um sinal preocupante: não é uma colisão estável e previsível, é uma ambiguidade que a pesquisa resolve de forma inconsistente. Agravante: `V3` é o próprio prompt que o `llms.txt` da destaque.ai lista como um dos seus serviços ("outsourcing de GEO"), pelo que a colisão ameaça directamente um prompt que a empresa trata como Mandatory na sua própria auditoria.
...
- **Prioridade:** P1 (subida de P2, ver actualização 2026-09-21: volume triplicou e apareceu uma segunda assinatura relacionada)
- **Esforço:** 2-4h (diagnóstico) + variável (correção: pode ser só reposição/upgrade de quota, ou correção do payload de campo)
- **Origem:** auditoria semanal 2026-08-03 (`mcp__Vercel__get_runtime_errors`, projeto `destaque-ai-tracker`, janela 7 dias); revisto 2026-08-10, 2026-08-24, 2026-09-07, 2026-09-14, 2026-09-21
- **Estado:** IN PROGRESS
...
- **Ação:** Confirmar em duas execuções seguidas que `Invalid Field: 'language_name'` não volta antes de marcar DONE: a "resolução" de 10 ago revelou-se prematura, não repetir o mesmo erro de leitura. **Atualização 2026-09-21:** investigar se `[surfaces] DataForSEO falhou, a cair para SerpApi` e `[audit-engine] google_aio/dataforseo (augmented) failed: This operation was aborted` (nova assinatura, ver Descrição) partilham a mesma causa raiz do lado do fornecedor.
...
- **Notes:** Já foi marcado prematuramente como resolvido uma vez (10 ago): por isso a barra de verificação agora exige duas confirmações seguidas, não uma. **Atualização 2026-09-14:** a assinatura `[surfaces] DataForSEO falhou, a cair para SerpApi: Internal SE Server Error` continua sem a assinatura antiga (`Invalid Field`), mas o volume cumulativo cresce (35 ocorrências desde 31 jul, era 22 em 07 set): o fallback funciona, mas a falha de origem do fornecedor não desaparece. **Atualização 2026-09-21: agrava, sobe de volta a P1.** `DataForSEO falhou, a cair para SerpApi` sobe para **103 ocorrências** (era 35), agora a maior assinatura acumulada da auditoria excepto o Perplexity, com ocorrência ainda esta manhã (07:09 UTC). Assinatura nova e relacionada: `google_aio/dataforseo (augmented) failed: This operation was aborted`, **63 ocorrências**, activa desde 11 set até esta manhã. Sete semanas em aberto sem resolução de fundo: o fallback continua funcional (não é um crash para o cliente), mas a falha de origem cresce, não diminui.
...
- **Prioridade:** P3
- **Esforço:** 30min-1h
- **Origem:** auditoria semanal 2026-09-21 (`mcp__Vercel__get_runtime_errors`, projeto `destaque-ai-tracker`, janela 7 dias)
- **Estado:** TODO
- **Descrição:** `Error: Attempted to call iniciaisDe() from the server but iniciaisDe is on the client. It's not possible to invoke a client function from the server...`, rotas `/settings` e `/settings.rsc`, 9 ocorrências, 1 utilizador, todas concentradas em 2026-09-15 entre 21:48–21:49 UTC. Sem nova ocorrência desde então (6 dias sem recorrência até esta auditoria).
- **Ação:** Localizar `iniciaisDe()` e confirmar se está correctamente isolada num Client Component ou se precisa de ser movida/serializada, à semelhança do bug já resolvido em `/site`/`/prompts` (ver item DONE de 2026-09-07).
- **Verificação:** `mcp__Vercel__get_runtime_errors` não mostra novas ocorrências numa janela de 7 dias (já cumprido nesta auditoria; falta confirmar se é por correcção ou por baixo tráfego na página).
- **Notes:** Baixo volume, um único utilizador, sem recorrência: pode já estar corrigido por um dos ~15 deploys de produção desta semana, ou pode ser um caminho de código raramente exercitado. Baixa urgência.
...
- **Origem:** auditoria semanal 2026-09-14 (`mcp__Vercel__get_runtime_errors`, projeto `destaque-ai-tracker`, janela 7 dias)
- **Estado:** DONE (2026-09-21, por ausência de recorrência)
...
- **Notes:** Baixo volume, primeira vez visto: tratar como watch, não como incêndio. **Atualização 2026-09-21:** nenhuma das duas assinaturas aparece na lista de 14 grupos de erro desta semana: sem recorrência em 7 dias, fechado por ausência de sintoma (não por correcção confirmada). Achado relacionado, novo e também de baixo risco: um cluster isolado de timeout em `deepseek/deepseek-v4-flash (knowledge)`, 144 ocorrências mas todas concentradas numa janela de 18 minutos a 14 set (18:52–19:10 UTC), consistente com um incidente pontual do lado do vendor nesse dia; sem nova ocorrência desde então. Não reaberto como item novo dado o padrão idêntico (pico isolado, sem recorrência).
