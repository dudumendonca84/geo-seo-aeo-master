...
> (Corrigido 17 Aug 2026 — a URL apontava para `skills/geo-seo-aeo-master/references/models.md`, um caminho que nunca existiu no histórico `git` deste repositório; o repositório em si é a raiz do skill. Ver `destaque-ai-self/improvements-backlog.md` item PROCESS.)
...
**Last refresh: 17 Aug 2026 (partial — one confirmed default-model change)** — Weekly self-audit routine refresh. Direct `WebFetch` to primary sources (openai.com, help.openai.com) returned `EGRESS_BLOCKED` from this execution environment (network egress proxy denies the domain outright, distinct from the HTTP 403 bot-protection response seen in earlier weeks — same practical effect: no primary-source fetch possible). `WebSearch` surfaced a **materially different signal than prior weeks**: three independent secondary outlets (TheNextWeb, Neowin, allblogthings.com), each citing consistent specific details (model name, ~62% fewer factual errors, unlimited free-tier chats, timing "this week" as of their ~6-9 Aug 2026 publish dates), report that OpenAI switched the ChatGPT **Free/Go default from GPT-5.5 Instant to GPT-5.6 Luna** in early August 2026. Three outlets independently converging on the same specific numeric claim is stronger evidence than the single-source absorptions this file has made before (e.g. the Perplexity/Opus 5 note below), but it is still secondary-sourced — no openai.com or help.openai.com page was directly read. Absorbed into the OpenAI table below with this caveat attached; **flag for confirmation against a primary source next refresh**, and downgrade this row if a primary source contradicts it. Scope note: the sourced claims describe the ChatGPT chat product default, not specifically the ChatGPT Search surface — left the Search-specific line unchanged pending clearer sourcing (see OpenAI § Products below). No other vendor default-model change found this week — Google AI Mode confirmed still on Gemini 3.5 Flash (unchanged, WebSearch corroborates), Bing Copilot confirmed still GPT-5 (unchanged), Perplexity's Claude Opus 5 adoption (absorbed 03 Aug from a single TechCrunch source) remains **unconfirmed by a primary source** as of this refresh — WebSearch this week returned only further secondary summaries repeating the same claim, none citing Perplexity's own changelog directly.

Prior entry — **Last refresh: 03 Aug 2026 (attempted, no confirmed default-model change)** — Weekly self-audit routine attempted the mandatory >7-day refresh before running the prompt test. Direct `WebFetch` to all 7 Tier 1 primary sources (openai.com/news, anthropic.com/news, blog.google/products/search, blogs.bing.com/search, developers.google.com/search/blog, deepmind.google/discover/blog, perplexity.ai/changelog) returned HTTP 403 (bot-protection) from this execution environment. `WebSearch` was used as a fallback and returned only third-party aggregator sites (blog.mean.ceo, tech-insider.org, mungomash.com, releasebot.io and similar), several of which contradict each other and contradict already-confirmed facts in this file (one claimed "GPT-5.3 Instant is default for every user", another claimed "Gemini 3.1 Pro" is current — both inconsistent with the confirmed GPT-5.6/GPT-5.5-Instant and Gemini 3.5 state below). Per this file's own sourcing discipline, none of these low-confidence, contradictory secondary claims were used to alter any row. **No material default-model change was confirmed with sufficient confidence this week.** One behavior change was absorbed from a credible secondary source: Perplexity began using **Claude Opus 5** in Search and Computer during the week of 24 Jul 2026 (TechCrunch, via `daily-agent/news-feed.md` entry 2026-07-31) — added as a note to the Perplexity table below; not used to change the Deck Builder `production`/`cost_optimized` mapping without primary-source confirmation. Flagging primary-source access as an operational gap for the routine (see `destaque-ai-self/improvements-backlog.md`), not a vendor-side gap.
...
| **GPT-5.6 — Luna** (`gpt-5.6-luna`) | 9 Jul 2026 (GA) | 256k tokens | Yes (with tool) | **New ChatGPT Free/Go default since ~6 Aug 2026** (secondary sources, see refresh note above — not primary-source confirmed), superseding GPT-5.5 Instant; also selectable in ChatGPT Work, API |
| **GPT-5.5 Instant** | May 2026 | 256k tokens | Yes (with tool) | Superseded as ChatGPT Free/Go default by GPT-5.6 Luna (~6 Aug 2026, secondary sources); still selectable/fallback |
...
- **ChatGPT Search / SearchGPT** — since 9 Jul 2026, default is tier-dependent: GPT-5.6 Sol for Plus/Pro/Business/Enterprise. Free/Go was GPT-5.5 Instant through end-Jul; secondary sources (see refresh note above) report the general ChatGPT chat default for Free/Go moved to **GPT-5.6 Luna** around 6 Aug 2026 — **not yet confirmed whether this specifically covers the ChatGPT Search surface** or only regular chat; treat the Search-specific default as GPT-5.6 Luna directionally, GPT-5.5 Instant as the last primary-confirmed value, until a primary source clarifies. Web search via Bing index + OpenAI crawl for both. Source: [openai.com/index/gpt-5-6](https://openai.com/index/gpt-5-6/) (9 Jul 2026); publish-date and detail cross-confirmed via OpenAI Help Center, TechCrunch, CNBC (direct fetch of openai.com blocked at verification time — HTTP 403 in Jul, `EGRESS_BLOCKED` in Aug).
